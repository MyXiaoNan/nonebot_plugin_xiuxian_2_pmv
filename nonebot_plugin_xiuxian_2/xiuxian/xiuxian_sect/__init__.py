...
9、宗主传位:宗主可以传位宗门成员
10、升级攻击修炼:升级道友的攻击修炼等级,每级修炼等级提升4%攻击力,可以加等级参数
11、宗门列表:查看所有宗门列表
    ...
    if sect_id:
        ...
        if sect_position == owner_position:
            ...
            if elixir_room_level_up_use_stone_cost > int(sect_info['sect_used_stone']):
                msg = "宗门可用灵石不满足升级条件，当前升级需要消耗宗门灵石：{}枚！".format(elixir_room_level_up_use_stone_cost)
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
                ...
                msg = "宗门建设度不满足升级条件，当前升级需要消耗宗门建设度：{}点！".format(elixir_room_level_up_sect_scale_cost)
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
                ...
                msg = "宗门消耗：{}建设度，{}宗门灵石\n".format(elixir_room_level_up_sect_scale_cost, elixir_room_level_up_use_stone_cost)
                msg += "成功升级宗门丹房，当前丹房为：{}!".format(elixir_room_level_up_config[str(to_up_level)]['name'])
    ...
    if sect_id:
        ...
        if sect_position == 4:
            ...
            if int(user_info['sect_contribution']) < elixir_room_config['领取贡献度要求']:
                msg = "道友的宗门贡献度不满足领取条件，当前宗门贡献度要求：{}点！".format(elixir_room_config['领取贡献度要求'])
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
            ...
            if sect_info['sect_materials'] < elixir_room_cost:
                msg = "当前宗门资材无法维护丹房，请等待{}点发放宗门资材后尝试领取！".format(config['发放宗门资材']['时间'])
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
    ...
    if sect_id:
        ...
        if sect_position == 4:
            ...
            if sect_info['sect_materials'] >= materialscost:
                ...
                msg = "本次学习消耗{}宗门资材，成功学习到本宗{}功法：{}\n{}".format(materialscost, mainbufftype, 
                                                                                    mainbuff['name'], mainbuffmsg)
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
                ...
                msg = "本次学习需要消耗{}宗门资材，不满足条件！".format(materialscost)
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
        ...
        if sect_position == owner_position:
            ...
            if sect_info['sect_used_stone'] >= stonecost and sect_info['sect_materials'] >= materialscost:
                if random.randint(0, 100) <= mainbuffconfig['获取到功法的概率']:
                    ...
                    msg = "本次搜寻消耗{}宗门灵石，{}宗门资材，成功获取到{}功法：{}\n{}".format(stonecost, materialscost, 
                                                                                        mainbufftype, mainbuff['name'], mainbuffmsg)
                    if XiuConfig().img:
                        pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                    else:
                    ...
                    msg = "本次搜寻消耗{}宗门灵石，{}宗门资材，可惜失败了！".format(stonecost, materialscost)
                    if XiuConfig().img:
                        pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                    else:
                ...
                msg = "本次搜寻需要消耗{}宗门灵石，{}宗门资材，不满足条件！".format(stonecost, materialscost)
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
        ...
        if sect_position == owner_position:
            ...
            if sect_info['sect_used_stone'] >= stonecost and sect_info['sect_materials'] >= materialscost:
                if random.randint(0, 100) <= secbuffconfig['获取到神通的概率']:
                    ...
                    msg = "本次搜寻消耗{}宗门灵石，{}宗门资材，成功获取到{}神通：{}\n{}".format(stonecost, materialscost, 
                                                                        secbufftype, secbuff['name'], secmsg)
                    if XiuConfig().img:
                        pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                    else:
    ...
    if sect_id:
        ...
        if sect_position == 4:
            ...
            if sect_info['sect_materials'] >= materialscost:
                ...
                secmsg = get_sec_msg(secbuff)
                msg = "本次学习消耗{}宗门资材，成功学习到本宗{}神通：{}\n{}：{}".format(materialscost, secbufftype, 
                                                                                secbuff['name'], secbuff['name'], secmsg)
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
...
async def upatkpractice_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if sect_id:
        ...
        if useratkpractice >= sect_level:
            msg = "道友的攻击修炼等级已达到当前宗门修炼等级的最高等级：{}，请捐献灵石提升贡献度吧！".format(sect_level)
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
            else:
        ...
        if int(user_info['stone']) < total_stone_cost:
            msg = "道友的灵石不够，升级到攻击修炼等级 {} 还需 {} 灵石!".format(
            useratkpractice + level_up_count, total_stone_cost - int(user_info['stone'])
            )
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        ...
        if sect_materials < total_materials_cost:
            msg = "道友的所处的宗门资材不足，还需 {} 资材来升级到攻击修炼等级 {}!".format(
            total_materials_cost - sect_materials, useratkpractice + level_up_count
            )
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        ...
        msg = "升级成功，道友当前攻击修炼等级：{}，消耗灵石：{}枚，消耗宗门资材{}!".format(
        useratkpractice + level_up_count, total_stone_cost, total_materials_cost
        )
        if XiuConfig().img:
            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
...
async def sect_task_refresh_(bot: Bot, event: GroupMessageEvent):
    ...
    if sect_id:
        if isUserTask(user_id):
            ...
            msg = "已刷新，道友当前接取的任务：{}\n{}".format(
            userstask[user_id]['任务名称'], userstask[user_id]['任务内容']['desc']
            )
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
...
async def sect_list_(bot: Bot, event: GroupMessageEvent):
    ...
    for sect in sect_lists_with_members:
        ...
        msg_list.append("编号{}：{}\n宗主：{}\n宗门建设度：{}\n成员数：{}".format(
        sect_id, sect_name, user_name, number_to(sect_scale), member_count))
...
async def sect_users_(bot: Bot, event: GroupMessageEvent):
    ...
    if user_info:
        ...
        if sect_id:
            ...
            msg = "☆【{}】的成员信息☆\n".format(sect_info['sect_name'])
            msg_list.append(msg)
            i = 1
            for user in userlist:
                msg = "编号{}:{},{}，宗门职位：{}，宗门贡献度：{}\n".format(
                i, user['user_name'], user['level'], jsondata.sect_config_data()["{}".format(user['sect_position'])]['title'], 
                user['sect_contribution']
                )
                msg += "QQ号{}".format(user['user_id'])
                msg_list.append(msg)
                i += 1
        else:
...
async def sect_task_(bot: Bot, event: GroupMessageEvent):
    ...
    if sect_id:
        ...
        if user_now_num >= config["每日宗门任务次上限"]:
            msg = "道友已完成{}次，今日无法再获取宗门任务了！".format(user_now_num)
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
            else:
        ...
        create_user_sect_task(user_id)
        msg = "{}".format(userstask[user_id]['任务内容']['desc'])
        if XiuConfig().img:
            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
...
async def sect_task_complete_(bot: Bot, event: GroupMessageEvent):
    ...
    if sect_id:
        ...
        if userstask[user_id]['任务内容']['type'] == 1:  # type=1：需要扣气血，type=2：需要扣灵石
            ...
            msg += "道友大战一番，气血减少：{}，获得修为：{}，所在宗门建设度增加：{}，资材增加：{}, 宗门贡献度增加：{}".format(
            costhp, get_exp, sect_stone, sect_stone * 10, int(sect_stone)
            )
            userstask[user_id] = {}
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
            ...
            if costls > int(user_info['stone']):
                msg = (
                    "道友兴高采烈的出门做任务，结果发现灵石带少了，当前任务所需灵石：{},"
                    "道友只好原路返回，浪费了一次出门机会，看你这么可怜，就不扣你任务次数了！".format(costls))
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
            ...
            sql_message.update_user_sect_task(user_id, 1)
            sql_message.update_user_sect_contribution(user_id, user_info['sect_contribution'] + int(sect_stone))
            msg = "道友为了完成任务购买宝物消耗灵石：{}枚，获得修为：{}，所在宗门建设度增加：{}，资材增加：{}, 宗门贡献度增加：{}".format(
            costls, get_exp, sect_stone, sect_stone * 10, int(sect_stone))
            userstask[user_id] = {}
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
    ...
    if give_qq:
        if give_qq == user_id:
            ...
            if give_user['sect_id'] == user_info['sect_id']:
                ...
                msg = "传老宗主{}法旨，即日起由{}继任{}宗主".format(
                    user_info['user_name'], give_user['user_name'], sect_info['sect_name'])
                ...
                msg = "{}不在你管理的宗门内，请检查。".format(give_user['user_name'])
                if XiuConfig().img:
                    pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                else:
...
async def sect_rename_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if user_info['sect_position'] != owner_position:
        ...
        if len_sect_name > 20:
            ...
            msg = "道友宗门灵石储备不足，还需{}灵石!".format(
                number_to(XiuConfig().sect_rename_cost - sect_info['sect_used_stone']))
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
            else:
...
(1)创建者境界最低要求为{}
(2)花费{}灵石费用
(3)创建者当前处于无宗门状态。道友暂未满足所有条件，请逐一核实后，再来寻我。""".format(
    XiuConfig().sect_min_level, XiuConfig().sect_create_cost)
        ...
        if sect_name:
            ...
            msg = "恭喜{}道友创建宗门——{}，宗门编号为{}。为道友贺！为仙道贺！".format(
                user_info['user_name'], sect_name, new_sect['sect_id'])
        else:
            ...
            if give_user['sect_id'] == user_info['sect_id']:
                ...
                if user_info['sect_position'] <= idx_position:
                    if give_user['sect_position'] <= user_info['sect_position']:
                        msg = "{}的宗门职务为{}，不在你之下，无权操作。".format(
                            give_user['user_name'], jsondata.sect_config_data()["{}".format(give_user['sect_position'])]['title'])
                        if XiuConfig().img:
                            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                        else:
                        ...
                        msg = "传{}{}法旨，即日起{}被{}除名".format(
                            jsondata.sect_config_data()["{}".format(user_info['sect_position'])]['title'], user_info['user_name'], 
                            give_user['user_name'], sect_info['sect_name']
                        )
                        if XiuConfig().img:
                            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                    ...
                    msg = "你的宗门职务为{}，只有长老及以上可执行踢出操作。".format(
                        jsondata.sect_config_data()["{}".format(user_info['sect_position'])]['title']
                    )
                    if XiuConfig().img:
                        pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                        await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
...
async def sect_out_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if user_info['sect_position'] != owner_position:
        ...
        msg = "道友已退出{}，今后就是自由散修，是福是祸，犹未可知。".format(sect_info['sect_name'])
        if XiuConfig().img:
            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
...
async def sect_donate_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if len(donate_num) > 0:
        if int(donate_num[0]) > user_info['stone']:
            msg = "道友的灵石数量小于欲捐献数量{}，请检查".format(int(donate_num[0]))
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
            else:
            ...
            msg = "道友捐献灵石{}枚，宗门建设度增加：{}，宗门贡献度增加：{}点，蒸蒸日上！".format(
                int(donate_num[0]), int(donate_num[0]), int(donate_num[0]))
            if XiuConfig().img:
                pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
            else:
...
async def sect_position_update_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    idx_position = int(position_zhanglao[0]) if len(position_zhanglao) == 1 else 1
    if user_info['sect_position'] > idx_position:
        msg = "你的宗门职位为{}，无权进行职位管理！".format(jsondata.sect_config_data()["{}".format(user_info['sect_position'])]['title'])
        if XiuConfig().img:
            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
    ...
    if give_qq:
            ...
            if len(position_num) > 0 and position_num[0] in list(jsondata.sect_config_data().keys()):
                ...
                if give_user['sect_id'] == user_info['sect_id'] and give_user['sect_position'] > user_info['sect_position']:
                    if int(position_num[0]) > user_info['sect_position']:
                        ...
                        msg = """传{}{}法旨:即日起{}为本宗{}""".format(
                            jsondata.sect_config_data()["{}".format(user_info['sect_position'])]['title'], 
                            user_info['user_name'], give_user['user_name'], 
                            jsondata.sect_config_data()["{}".format(int(position_num[0]))]['title'])
                        if XiuConfig().img:
                            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
                            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
                        else:
        ...
        msg = """请按照规范进行操作,ex:宗门职位变更2@XXX,将XXX道友(需在自己管理下的宗门)的变更为{}""".format(
            jsondata.sect_config_data().get('2', {'title': '没有找到2品阶'})['title'])
        if XiuConfig().img:
            pic = await get_msg_pic("@{}\n".format(event.sender.nickname) + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
...
async def join_sect_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if not user_info['sect_id']:
        ...
        if not sect_no.isdigit():
            ...
            msg = "欢迎{}师弟入我{}，共参天道。".format(user_info['user_name'], new_sect['sect_name'])
    else:
...
宗门贡献度：{}
宗门丹房：{}
""".format(user_name, sect_info['sect_name'], sect_id, sql_message.get_user_message(sect_info['sect_owner'])['user_name'], 
           jsondata.sect_config_data()["{}".format(sect_position)]["title"], number_to(sect_info['sect_scale']), 
           sect_info['sect_fairyland'] if sect_info['sect_fairyland'] else "暂无", top_idx_list.index(sect_id) + 1, 
           number_to(sect_info['sect_materials']), number_to(user_info['sect_contribution']), elixir_room_name)
        if sect_position == owner_position:
            msg += "\n宗门储备：{}灵石".format(sect_info['sect_used_stone'])
    else:
        msg = "一介散修，莫要再问。"
